...
AI 心理健康助手 —— 基于 Vue 3 + Spring Boot 的全栈心理健康服务平台，提供 AI 心理咨询、情绪日记、心理健康知识库等功能，并配备后台数据管理面板。

## 功能概览

### 用户端
- **AI 心理咨询** —— 与 AI 心理助手实时对话，获得情绪支持与建议（SSE 流式响应）
...
后端运行在 `http://localhost:8080`，首次启动会自动初始化管理员账号（admin/admin123）和测试数据。
...
```json
{
  "code": "200",    // "200"成功, "-1"登录过期, "500"业务异常
  "msg": "操作成功",
  "data": {}
}
```
...
后端预留了 `ChatAgent` 接口（`server/src/main/java/com/amha/agent/`），当前使用 `MockChatAgent` 模板回复实现。后续接入真实 LLM 时，只需实现该接口并替换 Spring Bean 即可，无需修改业务层代码。
...
共 28 个测试用例，覆盖用户认证、知识库 CRUD、情绪日记、SSE 流式对话、数据分析等全部模块。
