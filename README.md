...
│   ├── utils/               # 工具函数
│   └── views/               # 页面视图
├── server/                  # 后端源码 (Spring Boot 3 + JDK 17)
│   ├── pom.xml              # Maven 配置
...
Java 后端通过 `ChatAgent` 接口（`server/src/main/java/com/amha/agent/`）调用 Python Agent 服务。`RealChatAgent` 通过 WebFlux HTTP 调用 FastAPI 端点：
